...
<div class="row">
    ...
    @php
    $isNewProduct = session('new_product_id') == $producto->id_producto;
    @endphp
    <div class="col-lg-4 col-md-6 mb-4">
        <div class="card shadow-lg h-100 d-flex flex-column {{ $isNewProduct ? 'new-product' : '' }}"
        id="producto-{{ $producto->id_producto }}">
            ...
            <div class="card-img-top text-center p-3" style="height: 200px; overflow: hidden;">
                ...
                @else
                <!-- Mostrar un mensaje con ícono si no hay imágenes -->
                <div class="text-center text-muted">
                    <i class="fas fa-image fa-3x"></i>
                    <p class="mt-2">Sin imágenes</p>
                </div>
                @endif
            </div>
            ...
            <div class="card-body flex-grow-1">
                ...
                <p class="card-text"><strong>Sucursal:</strong>
                    {{ $producto->menu->sucursale->nombre_sucursal ?? 'No asignada' }}
                </p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Obtener el ID del nuevo producto desde la sesión
        let newProductId = "{{ session('new_product_id') }}";

        // Hacer scroll y resaltar el nuevo producto si existe
        if (newProductId) {
            let newProductElement = document.getElementById("producto-" + newProductId);
            
            if (newProductElement) {
                // Hacer scroll suave hasta el nuevo producto
                newProductElement.scrollIntoView({
                    behavior: "smooth",
                    block: "center"
                });

                // Agregar efecto de resaltado al nuevo producto
                newProductElement.classList.add("highlight");

                // Remover el efecto después de 4 segundos
                setTimeout(() => {
